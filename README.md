...
- **Alerts can reach a human.** With `[alerts] enabled = true`, runs that find anomalies, high load, or on-battery episodes append to `output/alerts.log`, and the tray icon raises a Windows toast for new lines (30-minute cooldown; history never re-notifies).
